                ...
                <div class="box-body">
                    <div class="nav-tabs-custom">
                        <ul class="nav nav-tabs">
                            <li class="active"><a href="#tab-shipping-address"
                                                  data-toggle="tab">{{ trans('order.shipping_address') }}</a></li>
                    ...
                    <table class="table table-condensed">
                        ...
                        <tr>
                            <td class="vertical-align-middle">
                                <img src="{{ Storage::disk('carriers')->exists($order->carrier->logo) ? (url(config('filesystems.disks.carriers.simple_path')).'/'.$order->carrier->logo) : (url(config('filesystems.disks.carriers.simple_path')).'/default.png') }}"
                    ...
                    <div class="col-md-12">
                        <table class="table table-striped table-hover">
                            ...
                            <tr>
                                ...
                                <th>{{ trans('product.attributes_tab') }}</th>
                            ...
                            @foreach($order->items as $item)
                                <tr>
                                    <td class="vertical-align-middle">
                                        <a href="{{ route('crud.products.edit', $item->product_id) }}">{{ $item->nameTranslated }}</a><br/>
                                        <span class="font-12">SKU: {{ $item->sku }}</span>
                                    </td>
                                    <td class="vertical-align-middle">{{ decimalFormat($item->price) . ' ' . $order->currency->name}}</td>
                                    <td class="vertical-align-middle">{{ decimalFormat($item->price) . ' ' . $order->currency->name }}</td>
                                    <td class="vertical-align-middle">
                                        @foreach($item->attributes as $attribute)
                                            {{$attribute['name']}}: {{$attribute['value']}}@if(!$loop->last), @endif
                                        @endforeach
                                    </td>
                                    <td class="vertical-align-middle">{{ $item->quantity }}</td>
                                    <td class="vertical-align-middle text-right">{{ decimalFormat($item->price* $item->quantity) . ' ' .  $order->currency->name }}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="col-md-6 col-md-offset-6">
                        <table class="table table-condensed">
                            <tr>
                                ...
                                <td class="text-right">{{ $order->total_shipping.' '.$order->currency->name }}</td>
                            </tr>
                            <tr>
                                <td class="text-right"><strong>{{ trans('common.total') }}:</strong></td>
                                <td class="text-right"><strong>{{ $order->total().' '.$order->currency->name }}</strong>
                                </td>
                            </tr>
                        </table>
                    </div>
