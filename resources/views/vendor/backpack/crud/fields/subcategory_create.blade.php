<!-- select2 from ajax -->
@php
    $connected_entity = new $field['model_custom'];
...
<div @include('crud::inc.field_wrapper_attributes') >
    ...
    >
        ...
        @if ($old_value)
            @php
                $item = $connected_entity->where($field['old_value'], "=", $old_value)->first();
            @endphp
            @if ($item)
                ...
                <option value=" {{ $item->{$field['old_value']} }}" selected>
...
@push('crud_fields_scripts')
    <script>
        jQuery(document).ready(function ($) {
            ...
            $("[name=category_id]").on("select2:select", function (e) {
                console.log($("[name=subcategory_id]"));
                $("[name=subcategory_id]").val([]).change();
            });
