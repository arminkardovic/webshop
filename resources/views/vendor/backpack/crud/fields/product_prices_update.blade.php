...
@push('crud_fields_scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            getAttributesUpdateTable(attributesSetSelect.val());

            attributesSetSelect.select2({
                theme: "bootstrap"
            }).on("change", function (e) {
                getAttributesUpdateTable($(this).val());
            });
        ...
        function getAttributesUpdateTable(setId) {
            var productId = {{$entry->id}};
            $.ajax({
                url: "{{ route('getUpdateProductPricesTable') }}",
                type: 'POST',
                data: {
                    setId: setId,
                    productId: productId
